...
public static function JSONParse (source, reviver : Function) { // avatar
    var result;
    ...
    if(reviver) {
        ...
        walk = function(holder, key) {
            ...
            var v;
            //--- avatar ---
            var holderArray : Boo.Lang.Hash = null;
            if(typeof holder == Array){
                holderArray = holder;
            }
            //---
            //var value = holder[key];
            var value = holderArray[key]; // avatar
            var valueArray : Boo.Lang.Hash = null; // avatar
            if(value && (typeof value == Boo.Lang.Hash || System.Type.GetType(value).IsArray)) {
                valueArray = value; // avatar
                //for(k in value) {
                for(k in valueArray){ // avatar
                    v = walk(value, k);
                    if(v != null) {
                        //value[k] = v;
                        valueArray[k] = v;
                        ...
                        //value.Remove(k);
                        valueArray.Remove(k); // avatar
                    }
                }
                value = valueArray; // avatar
            }
            //return reviver(holder, key, value);
            return reviver(holderArray, key, valueArray);
        };
        return walk({"": result}, "");
    } else {
        return result;
